...
import com.blusky.www.utils.HibernateUtils;
...
public class PropertyAction {
	...
	public String uploadProperty(
			...
			throws Exception {
		Set<UploadFiles> set = new HashSet<UploadFiles>();
		InputStream is = null;
		...
		if (result.hasErrors()) {
			...
			try {
				for (int i = 0; i < btnFile.length; i++) {
					...
					String ext = btnFile[i].getOriginalFilename().substring(
							btnFile[i].getOriginalFilename().lastIndexOf("."));
